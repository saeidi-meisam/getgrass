...
async def get_loop_count():
    ...
    """
    try:
        user_input = await asyncio.wait_for(async_input("لطفاً تعداد تلاش‌ها را وارد کنید (در صورت خالی بودن، حلقه بی‌نهایت ادامه خواهد یافت): ", 10.0), timeout=10.0)
...
async def connect_to_wss(socks5_proxy, user_id, loop_count):
    device_id = str(uuid.uuid4())  # استفاده از UUID4 برای سازگاری بیشتر
    ...
    while True:
        ...
        try:
            ...
            if hasattr(e, 'code') and e.code == 4000:
                ...
                break  # خروج از حلقه در صورت بروز خطای 4000
            await asyncio.sleep(5)  # تاخیر 5 ثانیه قبل از تلاش مجدد
