...
def search_ai_engine(query, engine_name, api_key):
    ...
    if engine_name.lower() == "perplexity":
        ...
        try:
            ...
            results = response.json().get("results", [])
            links = [result['url'] for result in results if 'url' in result]
            print(Fore.CYAN + f"You.com AI search for '{query}' returned {len(links)} links.")
            return links
        except Exception as e:
...
# Function to extract proxies by rendering JavaScript (without Selenium)
def fetch_proxies_with_js_render(link):
    ...
    try:
        ...
        print(Fore.MAGENTA + f"Fetched {len(proxies)} proxies with JS render from {link}.")
        return proxies
    except Exception as e:
        print(Fore.RED + f"Error fetching proxies with JS render from {link}: {e}")
        return []
    finally:
        session.close()
...
def search_unit(engine, query, max_depth=3, api_key=None):
    ...
    while depth < max_depth:
        ...
        # Extract proxies with JS rendering
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
...
def main():
    ...
    while True:
        ...
        time.sleep(random.uniform(300, 600))  # Wait for 5 to 10 minutes before the next cycle
